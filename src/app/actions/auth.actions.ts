...
export const resetPassword = async (value: z.infer<typeof ResetFormSchema>) => {
    ...
    try {
        ...
        if (!existingUser) {
            ...
            // so, the idea here is -> send the email to the resend id as I dont have a domain set up, so, only my resend accounts password could be reset in ShopTop, so to, reset password of any user, this step is taken
